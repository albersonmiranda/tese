...
Na notação matricial, essas estruturas são representadas pelo vetor $\mathbfit{y}_t$ $n$-dimensional com todas as observações no tempo $t$, pela matriz de soma $\mathbfit{S}$ de dimensão $n \times m$ que define as equações para todos os elementos da hierarquia, e pelo vetor $\mathbfit{b}_t$ composto pelas séries no nível mais desagregado.
